...
class CustomerController extends Controller
{
    ...
    public function customerData(Request $request)
    {
        ...
        foreach ($customers as $key => $customer) {
            ...
            // customer_details
            $nestedData['customer_details'] = $customer->name;
            if ($customer->company_name) $nestedData['customer_details'] .= '<br>' . $customer->company_name;
            if ($customer->email) $nestedData['customer_details'] .= '<br>' . $customer->email;
            $nestedData['customer_details'] .= '<br>' . $customer->phone_number . '<br>' . $customer->address . '<br>' . $customer->city;
            if ($customer->country) $nestedData['customer_details'] .= '<br>' . $customer->country;

            // discount_plan
            $nestedData['discount_plan'] = '';
            foreach ($customer->discountPlans as $index => $plan) {
                $nestedData['discount_plan'] .= $index ? ', ' . $plan->name : $plan->name;
            }
            ...
            if ($customer->points !== null) {
                $tier = RewardPointTier::where('min_points', '<=', $customer->points)
                    ->where('max_points', '>=', $customer->points)
                    ->first();
            }
            $nestedData['reward_tier'] = $tier ? $tier->name : '-';
            ...
            $nestedData['total_due'] = number_format($saleData->grand_total - $returned_amount - $saleData->paid_amount, 2) ?? 0;

            $deleteRequest = AccountRemovalRequest::where('user_id', $customer->id)->latest()->first();

            $nestedData['delete_request'] = $deleteRequest
                ? '<button class="btn btn-danger btn-sm delete-account-btn" data-customer-id="' . $customer->id . '" data-removal-id="' . $deleteRequest->id . '">Delete Request</button>'
                : '-';


            $nestedData['options'] = '<div class="btn-group"> <button type="button" class="btn btn-default btn-sm dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">' . trans("file.action") . ' <span class="caret"></span> <span class="sr-only">Toggle Dropdown</span> </button> <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">';
            if (in_array("customers-edit", $request['all_permission'])) {
                $nestedData['options'] .= '<li> <a href="' . route('customer.edit', $customer->id) . '" class="btn btn-link"><i class="dripicons-document-edit"></i> ' . __('db.edit') . '</a> </li>';
            }
            if (in_array("due-report", $request['all_permission'])) {
                $nestedData['options'] .= '<li> ' . \Form::open(['route' => 'report.customerDueByDate', 'method' => 'post', 'id' => 'due-report-form']) . ' <input type="hidden" name="start_date" value="' . date('Y-m-d', strtotime('-30 year')) . '" /> <input type="hidden" name="end_date" value="' . date('Y-m-d') . '" /> <input type="hidden" name="customer_id" value="' . $customer->id . '" /> <button type="submit" class="btn btn-link"><i class="dripicons-pulse"></i>' . __('db.Due Report') . '</button> ' . \Form::close() . ' </li>';
            }
            $nestedData['options'] .= '<li> <button type="button" data-id="' . $customer->id . '" class="clear-due btn btn-link" data-toggle="modal" data-target="#clearDueModal" ><i class="dripicons-brush"></i>' . __('db.Clear Due') . '</button> </li>';
            $nestedData['options'] .= '<li> <button type="button" data-id="' . $customer->id . '" class="deposit btn btn-link" data-toggle="modal" data-target="#depositModal" ><i class="dripicons-plus"></i>' . __('db.Add Deposit') . '</button> </li>';
            $nestedData['options'] .= '<li> <button type="button" data-id="' . $customer->id . '" class="getDeposit btn btn-link" ><i class="fa fa-money"></i>' . __('db.View Deposit') . '</button> </li>';
            if (in_array("customers-delete", $request['all_permission'])) $nestedData['options'] .= \Form::open(["route" => ["customer.destroy", $customer->id], "method" => "DELETE"]) . ' <li> <button type="submit" class="btn btn-link" onclick="return confirmDelete()"><i class="dripicons-trash"></i> ' . trans("file.delete") . '</button> </li>' . \Form::close() . ' </ul> </div>';
            $data[] = $nestedData;
        }
    ...
    public function getDestroyRequest(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'removal_id'   => 'required|integer',
        ...
        try {
            $removal = AccountRemovalRequest::where('id', $request->removal_id)
            ...
            return response()->json([
                ...
                'data'    => [
                    ...
                    'created_at' => $removal->created_at->format('Y-m-d H:i:s'),
                ]
            ], 200);
        } catch (Exception $e) {
            Log::error('Error fetching delete request: ' . $e->getMessage());
    ...
    public function confirmAccountDestroy(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'customer_id' => 'required|integer',
            'removal_id'  => 'required|integer',
        ...
        try {
            ...
            $removalRequest = AccountRemovalRequest::where('id', $request->removal_id)
                ->where('user_id', $customer->id)
                ->first();

            if (!$removalRequest) {
                return response()->json(['message' => 'Invalid removal request'], 400);
            }

            $sales = Sale::where('customer_id', $customer->id)->get();

            foreach ($sales as $sale) {
                if ($sale->document && Storage::exists($sale->document)) {
                    Storage::delete($sale->document);
                }

                $sale->products()->delete();
                // $sale->saleWarrantyGuarantees()->delete();

                if ($sale->tracking) {
                    $sale->tracking->histories()->delete();
                    $sale->tracking()->delete();
                }

                $sale->payments()->delete();
                $sale->returns()->delete();
                $sale->rewardPointUsage()->delete();
            }

            Sale::where('customer_id', $customer->id)->delete();

            Deposit::where('customer_id', $customer->id)->delete();

            RewardPointUsage::where('customer_id', $customer->id)->delete();
            ...
            return response()->json([
                'message' => 'Customer account and all related data deleted permanently.'
            ], 200);
        } catch (Exception $e) {

            DB::rollBack();
            Log::error('Account destroy error: ' . $e->getMessage());

            return response()->json([
                'message' => 'Failed to destroy account',
                'error'   => $e->getMessage(),
            ], 500);
        }
    }
