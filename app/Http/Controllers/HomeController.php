...
class HomeController extends Controller
{
    ...
    public function addonList()
    {
        ...
        if(!$role->hasPermissionTo('addons')) {
            return redirect('dashboard')->with('not_permitted', 'Sorry! You are not allowed to access this module');
        }
    ...
    public function recentPurchase()
    {
        ...
        else if(Auth::user()->role_id == 1)
        {
            ...
            $recent_purchase = Purchase::leftJoin('suppliers', 'suppliers.id', '=', 'purchases.supplier_id')->select('purchases.id','purchases.reference_no','purchases.status','purchases.created_at','purchases.grand_total','purchases.user_id','suppliers.name')->orderBy('id', 'desc')->where('purchases.supplier_id', Auth::id())->take(5)->get();
            return response()->json($recent_purchase);
        }
    }
