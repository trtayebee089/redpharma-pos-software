...
class ReturnController extends Controller
{
    ...
    // Sale data
    $lims_sale_data = Sale::select(
        'id', 'warehouse_id', 'customer_id', 'biller_id', 'currency_id', 'exchange_rate', 'sale_status'
    ...
    // Default Account
    $lims_account_data = Account::where('is_default', true)->first();
    $data['account_id'] = $lims_account_data->id ?? null;
    ...
    if ($request->hasFile('document')) {
        ...
        $request->document->move(public_path('documents/sale_return'), $documentName);
        $data['document'] = $documentName;
    }
    ...
    // Mail data
    $mail_data = [
        'email' => $lims_customer_data->email ?? null,
        'reference_no' => $lims_return_data->reference_no,
        'total_qty' => $lims_return_data->total_qty,
    ...
    foreach ($product_sale_ids as $product_sale_id) {
        $key = array_search($product_sale_id, $data['product_sale_id']);
        $pro_id = $data['product_id'][$key];
        $lims_product_data = Product::findOrFail($pro_id);
        ...
        if ($sale_unit[$key] != 'n/a') {
            // Sale unit
            $lims_sale_unit_data  = Unit::where('unit_name', $sale_unit[$key])->first();
            $sale_unit_id = $lims_sale_unit_data->id ?? 0;

            $quantity = $qty[$key];
            if ($lims_sale_unit_data) {
                if ($lims_sale_unit_data->operator == '*')
                    $quantity *= $lims_sale_unit_data->operation_value;
                elseif ($lims_sale_unit_data->operator == '/')
                    $quantity /= $lims_sale_unit_data->operation_value;
            }
            ...
            // Variant product
            if ($lims_product_data->is_variant) {
                $lims_product_variant_data = ProductVariant::FindExactProductWithCode($pro_id, $product_code[$key])->first();
                if ($lims_product_variant_data) {
                    ...
                    $lims_product_warehouse_data = Product_Warehouse::FindProductWithVariant(
                        $pro_id, $lims_product_variant_data->variant_id, $data['warehouse_id']
                    )->first();
                }
            }
            // Batch product
            ...
            // যদি warehouse data না থাকে তাহলে তৈরি করবো
            ...
            if ($lims_product_data->type == 'combo') {
                ...
                foreach ($product_list as $index => $child_id) {
                    $child_data = Product::find($child_id);
                    $child_variant_id = $variant_list[$index] ?? null;
                    ...
                    if (!$child_warehouse_data) {
                        $child_warehouse_data = Product_Warehouse::create([
                            'product_id' => $child_id,
                            'variant_id' => $child_variant_id,
                            'warehouse_id' => $data['warehouse_id'],
                            'qty' => 0
        ...
        // Mail data product info
        $mail_data['products'][$key] = $lims_product_data->is_variant
            ? $lims_product_data->name . ' [' . ($variant_data->name ?? '') . ']'
            : $lims_product_data->name;

        $mail_data['unit'][$key] = $sale_unit_id ? $lims_sale_unit_data->unit_code : '';
        $mail_data['qty'][$key] = $qty[$key];
        $mail_data['total'][$key] = $total[$key];

        // Product return insert
        ProductReturn::insert([
        ...
        // Update Product Sale return qty
        $product_sale_data = Product_Sale::where([
            ['product_id', $pro_id],
            ['sale_id', $data['sale_id']]
        ])->first();
