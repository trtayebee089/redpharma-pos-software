...
class RoleController extends Controller
{
    ...
    public function setPermission(Request $request)
    {
        ...
        if($request->has('riders-index')){
            $permission = Permission::firstOrCreate(['name' => 'riders-index']);
            if(!$role->hasPermissionTo('riders-index')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('riders-index');

        if($request->has('riders-add')){
            $permission = Permission::firstOrCreate(['name' => 'riders-add']);
            if(!$role->hasPermissionTo('riders-add')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('riders-add');

        if($request->has('riders-edit')){
            $permission = Permission::firstOrCreate(['name' => 'riders-edit']);
            if(!$role->hasPermissionTo('riders-edit')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('riders-edit');

        if($request->has('riders-delete')){
            $permission = Permission::firstOrCreate(['name' => 'riders-delete']);
            if(!$role->hasPermissionTo('riders-delete')){
                $role->givePermissionTo($permission);
            }
        }
        else
            $role->revokePermissionTo('riders-delete');
