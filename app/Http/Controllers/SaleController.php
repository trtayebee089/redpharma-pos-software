...
class SaleController extends Controller
{
    ...
    public function saleData(Request $request)
    {
        ...
        $q = Sale::join('payments', 'sales.id', '=', 'payments.sale_id')->whereDate('sales.created_at', '>=', $request->input('starting_date'))->whereDate('sales.created_at', '<=', $request->input('ending_date'))->select('sales.id', 'sales.*');
        ...
        if (empty($request->input('search.value'))) {
            $q = Sale::with('biller', 'customer', 'warehouse', 'user')->whereDate('sales.created_at', '>=', $request->input('starting_date'))->whereDate('sales.created_at', '<=', $request->input('ending_date'))->offset($start)->limit($limit)->orderBy($order, $dir);
            if (Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                $q = $q->where('user_id', Auth::id());
            } elseif (Auth::user()->role_id > 2 && config('staff_access') == 'warehouse') {
                $q = $q->where('warehouse_id', Auth::user()->warehouse_id);
            ...
            if (Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                $q = $q
                    ...
                    ->orwhere([['sales.reference_no', 'LIKE', "%{$search}%"], ['sales.user_id', Auth::id()]])
                    ->orwhere([['customers.name', 'LIKE', "%{$search}%"], ['sales.user_id', Auth::id()]])
                    ->orwhere([['customers.phone_number', 'LIKE', "%{$search}%"], ['sales.user_id', Auth::id()]])
                    ->orwhere([['billers.name', 'LIKE', "%{$search}%"], ['sales.user_id', Auth::id()]])
                    ->orwhere([['product_sales.imei_number', 'LIKE', "%{$search}%"], ['sales.user_id', Auth::id()]]);
                foreach ($field_names as $key => $field_name) {
                    $q = $q->orwhere([['sales.user_id', Auth::id()], ['sales.' . $field_name, 'LIKE', "%{$search}%"]]);
                }
            } elseif (Auth::user()->role_id > 2 && config('staff_access') == 'warehouse') {
                $q = $q
                    ->select('sales.*')
                    ->with('biller', 'customer', 'warehouse', 'user')
                    ->where('sales.user_id', Auth::id())
                    ->orwhere([['sales.reference_no', 'LIKE', "%{$search}%"], ['sales.warehouse_id', Auth::user()->warehouse_id]])
                    ->orwhere([['customers.name', 'LIKE', "%{$search}%"], ['sales.warehouse_id', Auth::user()->warehouse_id]])
                    ->orwhere([['customers.phone_number', 'LIKE', "%{$search}%"], ['sales.warehouse_id', Auth::user()->warehouse_id]])
                    ->orwhere([['billers.name', 'LIKE', "%{$search}%"], ['sales.warehouse_id', Auth::user()->warehouse_id]])
                    ->orwhere([['product_sales.imei_number', 'LIKE', "%{$search}%"], ['sales.warehouse_id', Auth::user()->warehouse_id]]);
                foreach ($field_names as $key => $field_name) {
                    $q = $q->orwhere([['sales.user_id', Auth::id()], ['sales.warehouse_id', Auth::user()->warehouse_id]]);
                }
            } else {
                $q = $q
                    ->select('sales.*')
        ...
        if (!empty($sales)) {
            foreach ($sales as $key => $sale) {
                ...
                $nestedData['key'] = $key;
                $nestedData['date'] = \Carbon\Carbon::parse($sale->created_at)
                    ->timezone('Asia/Dhaka')
                    ->format(config('date_format') . ' h:i a');

                $formatted_reference_no = preg_match('/\d{8}-\d{6}$/', $sale->reference_no, $matches) ? $matches[0] : '';
                $nestedData['reference_no'] = $formatted_reference_no;
                ...
                $nestedData['quantity'] = $sale
                    ->products()
                    ->selectRaw('SUM(product_sales.qty) as total_qty, product_sales.sale_unit_id, units.unit_code as unit_name')
                    ->join('units', 'units.id', '=', 'product_sales.sale_unit_id')
                    ->where('product_sales.sale_id', $sale->id)
                    ->groupBy('product_sales.sale_unit_id', 'units.unit_code')
                    ->get()
                    ->map(function ($row) {
                        $unitName = $row->unit_name ?? '';
                        return number_format($row->total_qty, 2) . ' ' . $unitName;
                    })
                    ->join('<br>');

                $nestedData['products'] = implode(
                    '<br>',
                    $sale->products
                        ->map(function ($product) use ($sale) {
                            $qty = DB::table('product_sales')->where('sale_id', $sale->id)->where('product_id', $product->id)->value('qty');

                            $saleUnitId = DB::table('product_sales')->where('sale_id', $sale->id)->where('product_id', $product->id)->value('sale_unit_id');

                            $unit = DB::table('units')->where('id', $saleUnitId)->value('unit_code');

                            $qty = $qty ? number_format($qty, 2) : '0.00';

                            return $product->name . ' (<strong>' . $qty . ' ' . ($unit ?? '') . '</strong>)';
                        })
                        ->toArray(),
                );
                ...
                if (config('is_packing_slip') && in_array('packing_slip_challan', $request['all_permission']) && ($sale->sale_status == 2 || $sale->sale_status == 5)) {
                    $nestedData['options'] .=
                        '<li>
                        <button type="button" class="create-packing-slip-btn btn btn-link" data-id = "' .
                        $sale->id .
                        '" data-toggle="modal" data-target="#packing-slip-modal"><i class="dripicons-box"></i> ' .
                        trans('file.Create Packing Slip') .
                        '</button>
                    </li>';
                }
                if (in_array('sale-payment-index', $request['all_permission'])) {
                    $nestedData['options'] .=
                        '<li>
                            <button type="button" class="get-payment btn btn-link" data-id = "' .
                        $sale->id .
                        '"><i class="fa fa-money"></i> ' .
                        trans('file.View Payment') .
                        '</button>
                        </li>';
                }
                if (in_array('sale-payment-add', $request['all_permission'])) {
                    $nestedData['options'] .=
                        '<li>
                            <button type="button" class="add-payment btn btn-link" data-id = "' .
                        $sale->id .
                        '" data-toggle="modal" data-target="#add-payment"><i class="fa fa-plus"></i> ' .
                        trans('file.Add Payment') .
                        '</button>
                        </li>';
                }
                if ($sale->sale_status !== 4) {
                    $nestedData['options'] .=
                        '<li>
                        <a href="return-sale/create?reference_no=' .
                        $nestedData['reference_no'] .
                        '" class="add-payment btn btn-link"><i class="dripicons-return"></i> ' .
                        trans('file.Add Return') .
                        '</a>
                    </li>';
                }
                ...
                $nestedData['options'] .=
                    '<li>
                    <button type="button" class="send-sms btn btn-link" data-id = "' .
                    $sale->id .
                    '" data-customer_id="' .
                    $sale->customer_id .
                    '" data-reference_no="' .
                    ...
                    '" data-payment_status="' .
                    $sale->payment_status .
                    '"  data-toggle="modal" data-target="#send-sms"><i class="fa fa-envelope"></i> ' .
                    trans('file.Send SMS') .
                    '</button>
                </li>';
                ...
                $nestedData['options'] .=
                    ...
                    trans('file.Whatsapp Notification') .
                    '
                        </button>
                    </form>
                </li>';
                ...
                $nestedData['options'] .=
                    '<li>
                        <button type="button" class="add-delivery btn btn-link" data-id = "' .
                    $sale->id .
                    '"><i class="fa fa-truck"></i> ' .
                    trans('file.Add Delivery') .
                    '</button>
                    </li>';
    ...
    public function saleData1(Request $request)
    {
        $columns = [
            1 => 'created_at',
            2 => 'reference_no',
        ...
        $baseQuery = Sale::query()
            ->with(['customer', 'biller', 'warehouse', 'user'])
            ->leftJoin('payments', 'sales.id', '=', 'payments.sale_id')
            ->whereBetween('sales.created_at', [$request->input('starting_date'), $request->input('ending_date')])
            ->select('sales.*');
        ...
        // Filter by fields
        foreach (['warehouse_id', 'sale_status', 'payment_status', 'sale_type'] as $filter) {
            if ($value = $request->input($filter)) {
                $baseQuery->where($filter, $value);
            }
        }

        if ($method = $request->input('payment_method')) {
            $baseQuery->where('payments.paying_method', $method);
        }
        ...
        if ($searchValue) {
            $baseQuery->where(function ($query) use ($searchValue) {
                $query
                    ->where('reference_no', 'like', "%{$searchValue}%")
                    ->orWhereHas('customer', fn($q) => $q->where('name', 'like', "%{$searchValue}%"))
                    ->orWhereHas('biller', fn($q) => $q->where('name', 'like', "%{$searchValue}%"));
            });
        }
        ...
        $data = $sales->map(function ($sale, $key) {
            return [
                'id' => $sale->id,
                'key' => $key,
                ...
                'sale_status' => trans("file.{$sale->sale_status}"),
                'payment_status' => trans("file.{$sale->payment_status}"),
                'options' => view('partials.sale_action_buttons', compact('sale'))->render(),
            ];
        });
    ...
    public function exportSalesExcel(Request $request)
    {
        try {
            ...
            foreach ($sales as $sale) {
                ...
                $sheet->setCellValue("C{$startRow}", $sale->biller->name);
    ...
    public function create()
    {
        ...
        if ($role->hasPermissionTo('sales-add')) {
            ...
            if (Auth::user()->role_id > 2) {
                ...
                $lims_biller_list = Biller::where([['is_active', true], ['id', Auth::user()->biller_id]])->get();
            } else {
                $lims_warehouse_list = Warehouse::where('is_active', true)->get();
    ...
    public function store(Request $request)
    {
        ...
        if (isset($request->reference_no)) {
            $this->validate($request, [
                'reference_no' => ['max:191', 'required', 'unique:sales'],
            ]);
        }

        $data['user_id'] = Auth::id();
        $cash_register_data = CashRegister::where([['user_id', $data['user_id']], ['warehouse_id', $data['warehouse_id']], ['status', true]])->first();
                ...
                if (!in_array('manufacturing', explode(',', config('addons')))) {
                    ...
                    foreach ($product_list as $key => $child_id) {
                        ...
                        if (count($variant_list) && $variant_list[$key]) {
                            $child_product_variant_data = ProductVariant::where([['product_id', $child_id], ['variant_id', $variant_list[$key]]])->first();

                            $child_warehouse_data = Product_Warehouse::where([['product_id', $child_id], ['variant_id', $variant_list[$key]], ['warehouse_id', $data['warehouse_id']]])->first();
            ...
            if ($sale_unit[$i] != 'n/a') {
                ...
                if ($data['sale_status'] == 1) {
                    ...
                    if ($lims_product_data->is_variant) {
                        ...
                        $lims_product_warehouse_data = Product_Warehouse::where([['product_batch_id', $product_batch_id[$i]], ['warehouse_id', $data['warehouse_id']]])->first();
    ...
    public function recentSale()
    {
        if (Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            $recent_sale = Sale::join('customers', 'sales.customer_id', '=', 'customers.id')
                ...
                ->where([['sales.sale_status', 1], ['sales.user_id', Auth::id()]])
                ->orderBy('id', 'desc')
                ->take(10)
                ->get();
    ...
    public function recentDraft()
    {
        if (Auth::user()->role_id > 2 && config('staff_access') == 'own') {
            $recent_draft = Sale::join('customers', 'sales.customer_id', '=', 'customers.id')
                ->select('sales.id', 'sales.reference_no', 'sales.customer_id', 'sales.grand_total', 'sales.created_at', 'customers.name')
                ->where([['sales.sale_status', 3], ['sales.user_id', Auth::id()]])
                ->orderBy('id', 'desc')
                ->take(10)
                ->get();
    ...
    public function createSale($id)
    {
        ...
        if ($role->hasPermissionTo('sales-edit')) {
            ...
            $lims_sale_data = Sale::find($id);
            $lims_product_sale_data = Product_Sale::where('sale_id', $id)->get();
            $lims_product_list = Product::where([['featured', 1], ['is_active', true]])->get();
    ...
    public function genInvoice($id)
    {
        ...
        $redirectUrl = request()->query('redirect', route('sales.index'));

        if ($lims_pos_setting_data->invoice_option == 'A4') {
            return view('backend.sale.a4_invoice', compact('lims_sale_data', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'paid_by_info', 'sale_custom_fields', 'customer_custom_fields', 'product_custom_fields', 'qrText', 'totalDue', 'redirectUrl'));
            ...
            return view('backend.sale.invoice58', compact('lims_sale_data', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'sale_custom_fields', 'customer_custom_fields', 'product_custom_fields', 'qrText', 'totalDue', 'redirectUrl'));
        } else {
            return view('backend.sale.invoice', compact('lims_sale_data', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'sale_custom_fields', 'customer_custom_fields', 'product_custom_fields', 'qrText', 'totalDue', 'redirectUrl'));
        }
    }
