...
<section class="forms">
    ...
    <div class="table-responsive">
        ...
        <button id="submitOrderTable" style="margin-left: 19px;" class="btn btn-primary">Submit Orders</button>
...
<script type="text/javascript">
    ...
    $('#submitOrderTable').on('click', function () {
        ...
        $('#product-report-table tbody tr').not('.d-none').each(function () {
            const row = $(this);
            const code = row.find('td:eq(0)').text().trim(); // name column
            const name = row.find('td:eq(1)').text().trim(); // name column
            const category = row.find('td:eq(2)').text().trim(); // category column
            const sold_qty = row.find('td:eq(3)').text().trim(); // sold_qty
            const in_stock = row.find('td:eq(4)').text().trim(); // in_stock
